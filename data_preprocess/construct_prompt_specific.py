...
def item_pred_one_cand_task(input_path, output_path,item_list,df,id_title,user_seq,k):
    ...
    for index, row in data.iterrows():
        ...
        for i in range(len(pos_neg_items)):
            ...
            input_prompt = f"User general preferences: [UserEmb]. Domain-specific interaction history: {history.strip()}. Candidate item: {pair_item_prompt}."
...
root_path = f'../datasets/{task}/{dataset}'
...
with open(f'../datasets_no_filter/{task}/{dataset}/user_seq.txt','r') as f:
    user_seq = json.load(f)

test_n_neg_samples = 19
train_n_neg_samples = 1
valid_n_neg_samples = 1

logger.info('save train')


item_pred_one_cand_task(f'../datasets/{task}/{dataset}/train.csv',
             f'../datasets/{task}/{dataset}/train_prompt.json',
                    item_list=item_list,df=df,id_title=id_title,user_seq = user_seq,k=train_n_neg_samples)
logger.info('save valid')
item_pred_one_cand_task(f'../datasets/{task}/{dataset}/valid.csv',
             f'../datasets/{task}/{dataset}/valid_prompt.json',
                    item_list=item_list,df=df,id_title=id_title,user_seq = user_seq,k=valid_n_neg_samples)
logger.info('save test')
item_pred_one_cand_task(f'../datasets/{task}/{dataset}/test.csv',
             f'../datasets/{task}/{dataset}/test_prompt.json',
                    item_list=item_list,df=df,id_title=id_title,user_seq = user_seq,k=test_n_neg_samples)
